		...
		class XMLParser
			...
			def metadata
				objs.remove_namespaces!
				{
					...
					:rights_and_pricing => parse_rights_and_pricing(objs)
				}
			end  
			...
			def parse_rights_and_pricing(objs)
				rights_and_pricing = RightsAndPricing.new
				rights_and_pricing.cleared_territories = []
				rights_and_pricing.sales_start_date = nil
				rights_and_pricing.intervals = []

				objs.search('/package/software/software_metadata/products/product').each do |prod|
					cleared_for_sale = prod.at_xpath('cleared_for_sale').text == 'true'
					territory = prod.at_xpath('territory').text
					if territory == "WW"
						rights_and_pricing.sales_start_date = Date.parse(prod.at_xpath('sales_start_date').text)
						prod.search('intervals/interval').each do |i|
							...
							rights_and_pricing.intervals << interval
						end	     	    	
					end
					rights_and_pricing.cleared_territories << territory if cleared_for_sale
				end
				rights_and_pricing
			end
